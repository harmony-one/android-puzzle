window.__require = function e(t, n, r) {
  ...
  Entry: [ function(require, module, exports) {
    ...
    cc.Class({
      ...
      setup: function setup(rank, key, score, medal, tx) {
        this.tx = tx;
  ...
  Game: [ function(require, module, exports) {
    ...
    cc.Class({
      ...
      onLoad: function onLoad() {
        ...
        this.reset();
        null != this.themeMusic && Global.isAndroid() && cc.audioEngine.playMusic(this.themeMusic, true);
      },
      ...
      getNodePosition: function getNodePosition(row, col) {
      ...
      onTouchEnd: function onTouchEnd(event) {
        ...
        if (Math.abs(deltaX) < SWIFT_DISTANCE && Math.abs(deltaY) < SWIFT_DISTANCE) {
          ...
          var touchBlock = this.findBlockByCoordinate(cc.v2(endPos.x - board.x, endPos.y - board.y));
          this.moveByClickingBlock(touchBlock);
        } else this.moveBySwift(deltaX, deltaY);
      },
      moveBySwift: function moveBySwift(deltaX, deltaY) {
        var direction = void 0;
        direction = Math.abs(deltaX) >= Math.abs(deltaY) ? deltaX > 0 ? "R" : "L" : deltaY > 0 ? "U" : "D";
        this.tryMove(direction);
      },
      moveByClickingBlock: function moveByClickingBlock(touchBlock) {
        cc.log("moveByClickingBlock", touchBlock);
        if (void 0 === touchBlock || null === touchBlock) return;
        var currentBlock = this.findBlock(this.selectedX, this.selectedY);
        var dx = currentBlock.x - touchBlock.x;
        var dy = currentBlock.y - touchBlock.y;
        var absDx = Math.abs(dx);
        var absDy = Math.abs(dy);
        if ((1 === absDx || 1 === absDy) && !(1 === absDx && 1 === absDy) && absDx <= 1 && absDy <= 1) {
          var direction = "";
          0 != dx ? direction = dx > 0 ? "L" : "R" : 0 != dy && (direction = dy > 0 ? "U" : "D");
          this.tryMove(direction);
      ...
      findBlockByCoordinate: function findBlockByCoordinate(v2) {
        for (var i = 0; i < this.listBlockScripts.length; i++) {
          var block = this.listBlockScripts[i];
          var blockPos = this.getNodePosition(block.x, block.y);
          var halfSize = cc.v2(this.nodeWidth, this.nodeHeight);
          var minX = blockPos.x - halfSize.x;
          var maxX = blockPos.x + halfSize.x;
          var minY = blockPos.y - halfSize.y;
          var maxY = blockPos.y + halfSize.y;
          cc.log("block" + i, blockPos, minX, maxX, minY, maxY);
          if (minX <= v2.x && v2.x <= maxX && minY <= v2.y && v2.y <= maxY) return this.listBlockScripts[i];
        }
  ...
  Global: [ function(require, module, exports) {
    ...
    window.Global = {
      ...
      restUpdateScore: function restUpdateScore(onSuccessCallback) {
        var url = "http://puzzlemobile.hmny.io:3000/api/submit";
        ...
        xhr.onreadystatechange = function() {
          ...
          if (4 === this.readyState && 200 === this.status) {
            ...
            if ("success" === status) onSuccessCallback(); else {
        ...
        xhr.onerror = function() {
          null != Global.loading && (Global.loading.active = false);
          var msg = "Networking problem \n Failed to save your score";
          Global.showAlertDialog(msg);
        };
        xhr.ontimeout = function(e) {
          null != Global.loading && (Global.loading.active = false);
          var msg = "Network: Request timeout.";
          Global.showAlertDialog(msg);
        };
        xhr.send(params);
      },
      restGetLeaderBoard: function restGetLeaderBoard(onSuccessCallback) {
        var url = "http://puzzlemobile.hmny.io:3000/api/leader_boards";
        var xhr = new XMLHttpRequest();
        ...
        xhr.onreadystatechange = function() {
          ...
          if (4 == xhr.readyState && xhr.status >= 200 && xhr.status < 400) {
            ...
            var data = JSON.parse(xhr.responseText);
            var status = data["status"];
            if ("success" === status) onSuccessCallback(response); else {
              var msg = "Unable to get \n Leader Board! \n Please try again.";
              Global.showAlertDialog(msg);
            }
          }
        };
      ...
      start: function start() {
        ...
        Global.restGetLeaderBoard(function(jsonText) {
          ...
          entries.forEach(function(entry) {
            var record = cc.instantiate(that.prefabEntry);
            var script = record.getComponent("Entry");
            ...
            var tx = "0x0816c249e4ecc3f9992044a8aaa4cc13cb3a5465a35cc52b5804b98170d77040";
            void 0 != entry.txn && null != entry.txn && (tx = entry.txn);
  ...
  Loading: [ function(require, module, exports) {
    ...
    cc.Class({
      ...
      onEnable: function onEnable() {
        var seq = cc.repeatForever(cc.rotateBy(1, 360).easing(cc.easeIn(3)));
